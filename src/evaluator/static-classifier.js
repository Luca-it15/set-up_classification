...
export function classifyDescription(report) {
  ...
  const summary = { overall_score: null, quality_score: null, coverage, checklist_score: null, coverage, passed: evaluated.filter(item => item.outcome === 'pass').length, failed: evaluated.filter(item => item.outcome === 'fail').length, evaluated: evaluated.length, applicable: applicableCount };

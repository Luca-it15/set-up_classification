...
export function redactSensitiveText(input, { anonymized = false, roots = [] } = {}) {
  ...
  value = value.replace(/(["']?(?:api[_-]?key|apikey|access[_-]?token|token|password|passwd|secret|client[_-]?secret|authorization|credential|github_token)["']?\s*[:=]\s*)(\[REDACTED\]|[^\s,}\]"']+)/gi, '$1[REDACTED]');
  if (anonymized) {

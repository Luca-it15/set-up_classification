...
assert.equal(report.extensions['org.awdf.contracts'].records.filter(r=>r.status==='contract_present').length,2);
let browser;
...
try {
 ...
 await page.locator('.atlas-detail .contract-panel .rule-card summary').filter({hasText:'codex'}).waitFor();
 await page.getByRole('button',{name:'Mostra tutta la rete',exact:true}).click();
 await page.getByLabel('Filtra per tool').selectOption({label:'Claude Code'});
 ...
 await page.getByRole('button',{name:'Mostra tutta la rete',exact:true}).click();
 await page.getByLabel('Filtra per tool').selectOption('all');
